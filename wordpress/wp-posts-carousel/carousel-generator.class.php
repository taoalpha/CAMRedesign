...
class WpPostsCarouselGenerator {
    ...
    public static function generate($atts) {
        ...
        */
        ...
        if ( $taxonomies = get_object_taxonomies($post_type, 'objects') ) {
            foreach( $taxonomies as $taxonomy ) {
                if ( preg_match('/\bcategory\b/i', $taxonomy->name) ) {
                    $post_type_category = $taxonomy->name;
                } else if ( preg_match('/\btag\b/i', $taxonomy->name) ) {
        ...
        */
        ...
        /** find the feature story under this category **/
        $cat = '';
        $catIDs = get_categories('child_of='.$catID);
        foreach ($catIDs as $curcid) {
          if($curcid->name == "Features"){
            $cat = $curcid ->term_id;
          }
        }
        /*
        ...
        while ( $loop->have_posts() ) {
            ...
            */
            if ( $params['show_description'] === 'excerpt' ) {
                $description = '<a href="' . $post_url . '" class=""' . $post->post_title . '">' ;
                $description .= '<div class="wp-posts-carousel-desc">' . get_the_excerpt() . '</div></a>';
            } else if ( $params['show_description'] === 'content' ) {
                $description = '<a href="' . $post_url . '" class=""' . $post->post_title . '">';
                $description .= '<div class="wp-posts-carousel-desc">' . ($params['allow_shortcodes'] === 'true' ? do_shortcode(get_the_content('', true)) : get_the_content()) . '</div></a>';
            }
